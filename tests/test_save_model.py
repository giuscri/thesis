...
def test_pca_when_reconstruction_is_saved(environ):
    ...
    assert os.path.exists(
        "/tmp/model/reconstruction/pca-filtered-model-100-components/"
    )
    assert os.path.exists(
        "/tmp/model/reconstruction/pca-filtered-model-100-components/weights.h5"
    )
    assert os.path.exists(
        "/tmp/model/reconstruction/pca-filtered-model-100-components/pca.pkl"
    )
    assert os.path.exists("/tmp/model/reconstruction/pca-filtered-model-20-components/")
    assert os.path.exists(
        "/tmp/model/reconstruction/pca-filtered-model-20-components/weights.h5"
    )
    assert os.path.exists(
        "/tmp/model/reconstruction/pca-filtered-model-20-components/pca.pkl"
    )
...
def test_pca_when_retraining_is_saved(environ):
    ...
    assert os.path.exists(
        "/tmp/model/retraining/pca-filtered-model-100-components/weights.h5"
    )
    assert os.path.exists(
        "/tmp/model/retraining/pca-filtered-model-100-components/pca.pkl"
    )
    assert os.path.exists("/tmp/model/retraining/pca-filtered-model-20-components/")
    assert os.path.exists(
        "/tmp/model/retraining/pca-filtered-model-20-components/weights.h5"
    )
    assert os.path.exists(
        "/tmp/model/retraining/pca-filtered-model-20-components/pca.pkl"
    )
...
def test_models_with_same_random_state_have_same_loss_and_accuracy(environ, mnist):
    ...
    with open(
        "/tmp/model/retraining/pca-filtered-model-20-components/pca.pkl", "rb"
    ) as f:
        pca = load(f)
