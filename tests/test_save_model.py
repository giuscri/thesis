...
def test_pca_when_reconstruction_is_saved(environ):
    ...
    assert os.path.exists(
        "/tmp/model/reconstruction/pca-filtered-model-100-components.h5"
    )
    assert os.path.exists(
        "/tmp/model/reconstruction/pca-filtered-model-20-components.h5"
    )
...
def test_models_with_same_random_state_have_same_loss_and_accuracy(environ, mnist):
    ...
    model = keras.models.load_model(
        "/tmp/model/retraining/pca-filtered-model-20-components.h5",
        custom_objects={"PCA": layers.PCA},
    )
