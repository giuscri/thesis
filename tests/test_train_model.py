...
def test_pca_when_retraining_is_saved(environ):
    command = ["python", "bin/train-model", "--retraining", "--epochs", "0",
               "--pca", "--n-components", "100", "20"]
    process = subprocess.run(command, stdout=subprocess.PIPE)
    assert process.returncode == 0
    ...
    created_files = [
        ...
        "/tmp/model/pca-filtered-model-20-components-reconstruction/weights.h5",
        "/tmp/model/pca-filtered-model-20-components-reconstruction/pca.pkl",
        "/tmp/model/pca-filtered-model-100-components-retraining/weights.h5",
        "/tmp/model/pca-filtered-model-100-components-retraining/pca.pkl",
        "/tmp/model/pca-filtered-model-20-components-retraining/weights.h5",
        "/tmp/model/pca-filtered-model-20-components-retraining/pca.pkl",
    ]
    for f in created_files:
        assert os.path.exists(f)
...
def test_models_with_same_random_state_have_same_loss_and_accuracy(environ, mnist):
    ...
    command = ["python", "bin/train-model", "--retraining", "--pca",
